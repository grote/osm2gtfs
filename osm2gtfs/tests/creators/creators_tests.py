...
class CreatorsTestsAbstract(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.required_values = self._get_required_values()

        # Initialize some standard paths and variables
        self.standard_values = self._get_standard_values()

        # Load configuration
        self.config = Configuration(
            CreatorsTestsArgs(self.standard_values['config_file'], self.selector))
    ...
    def test_refresh_routes_cache(self):
        ...
        cache_file = os.path.join(self.standard_values['data_dir'], self.selector + "-routes.pkl")
        mocked_overpass_data_file = self.standard_values['mocked_overpass_routes']
        ...
        self.assertEqual(
            len(routes), self.required_values['routes_count'],
            'Wrong count of routes in the cache file')

    def test_refresh_stops_cache(self):
        data = OsmConnector(self.config)
        cache_file = os.path.join(self.standard_values['data_dir'], self.selector + "-stops.pkl")
        mocked_overpass_data_file = self.standard_values['mocked_overpass_stops']
        if os.path.isfile(cache_file):
            os.remove(cache_file)
    ...
    def test_gtfs_from_cache(self):
        ...
        routes_cache_file = os.path.join(
            self.standard_values['data_dir'], self.selector + "-routes.pkl")
        stops_file = os.path.join(
            self.standard_values['data_dir'], self.selector + "-stops.pkl")
        self.assertTrue(
        ...
        gtfs_expected_result = self.standard_values['reference_gtfs']
        gtfs_generated_result = self.standard_values['output_file']

        # Verify GTFS
        self.assertTrue(
            CreatorsTestsHelper.is_valid_gtfs(
                gtfs_generated_result, self.required_values['gtfs_files']),
            'The generated GTFS is not valid')

        self.assertTrue(
            CreatorsTestsHelper.is_valid_gtfs(
                gtfs_expected_result, self.required_values['gtfs_files']),
        ...
        self.assertTrue(
            CreatorsTestsHelper.check_osm_route_stop_times(
                ...
                self.required_values['route_id_to_check']),
