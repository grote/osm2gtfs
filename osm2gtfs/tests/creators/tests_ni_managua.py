...
class Args():
    def __init__(self, config_file):
        ...
        self.selector = "ni_managua"
        self.output = os.path.realpath(
            os.path.join(current_dir, "../../../data/ni_managua_tests.zip"))


def is_valid_gtfs(gtfs):
    # Checking for present GTFS files
    gtfs_files = [
        "agency.txt", "stops.txt", "routes.txt", "trips.txt",
        "stop_times.txt", "calendar.txt", "feed_info.txt", "shapes.txt"
    ]
...
class TestCreatorsNiManagua(unittest.TestCase):
    def setUp(self):
        self.data_dir = os.path.realpath(os.path.join(current_dir, "../../../data/"))
        self.config_file = os.path.realpath(
            os.path.join(current_dir, "../../creators/ni_managua/config.json")
        )
        self.fixture_folder = os.path.join(os.path.realpath(current_dir), "fixtures/ni_managua/")
        ...
        self.config.data['schedule_source'] = os.path.join(
            self.fixture_folder, "timetable.json")
    ...
    def test_refresh_routes_cache(self):
        ...
        cache_file = os.path.join(self.data_dir, "ni_managua-routes.pkl")
        ...
        cache = Cache()
        routes = cache.read_data('ni_managua-routes')
        self.assertEqual(len(routes), 45, 'Wrong count of routes in the cache file')

    def test_refresh_stops_cache(self):
        data = OsmConnector(self.config)
        cache_file = os.path.join(self.data_dir, "ni_managua-stops.pkl")
        ...
        cache = Cache()
        stops = cache.read_data('ni_managua-stops')
        amount_of_stops = len(stops['regular']) + len(stops['stations'])
        self.assertEqual(amount_of_stops, 1993, 'Wrong count of stops in the cache file')

    def test_gtfs_from_cache(self):
        # the cache is generated by the previous two functions
        routes_cache_file = os.path.join(self.data_dir, "ni_managua-routes.pkl")
        stops_file = os.path.join(self.data_dir, "ni_managua-stops.pkl")
        self.assertTrue(
        ...
        gtfs_expected_result = os.path.join(self.fixture_folder, "ni-managua-gtfs.zip.ref")
        gtfs_generated_result = os.path.join(self.data_dir, "ni_managua_tests.zip")
        ...
        self.assertEqual(gtfs_infos["stop_points_count"], 1446,
                         "Wrong stop_points count in the generated GTFS")
        self.assertEqual(gtfs_infos["stop_areas_count"], 547,
                         "Wrong stop_areas count in the generated GTFS")
        self.assertEqual(gtfs_infos["routes_count"], 45,
                         "Wrong routes count in the generated GTFS")
        self.assertTrue(
            check_osm_route_stop_times(gtfs_expected_result, gtfs_generated_result, 111),
            "Error found on stop_times for the route 111"
        )
